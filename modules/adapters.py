...
class TextAdapterIdentityInit(TextAdapter):
    @register_to_config
    def __init__(self, in_dim, int_dim, out_dim):
        super().__init__(in_dim, int_dim, out_dim)
        self.alpha = nn.Parameter(torch.tensor([0.01]))

    def forward(self, x):
        y = self.ff1(self.norm1(x))
        y = self.ff2(self.norm2(y))
        # a = self.alpha**2
        a = self.alpha
        return TextAdapterOutput(x + a * y)


class IdentityTextAdapter(TextAdapter):
    @register_to_config
    def __init__(self, in_dim, int_dim, out_dim):
        super().__init__(in_dim, int_dim, out_dim)
        self.alpha = nn.Parameter(torch.tensor([0.01]))
