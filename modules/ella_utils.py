...
class ELLAProxyUNet(torch.nn.Module):
    ...
    def forward(
        ...
        # encoder_hidden_states_general,encoder_hidden_states_object,encoder_hidden_states_style,encoder_hidden_states_color = torch.chunk(time_aware_encoder_hidden_states,4)
        # print(f"{time_aware_encoder_hidden_states.shape=}")
        uncond , cond = torch.chunk(time_aware_encoder_hidden_states,2)
        # print(f"{cond.shape=}")
        # uncond , _ , _ , _ = torch.chunk(uncond,4)
        uncond = torch.stack([ten for ten in torch.chunk(uncond,4)],dim=1)
        cond = torch.stack([ten for ten in torch.chunk(cond,4)],dim=1)
        # print(f"{cond.shape=}")
        time_aware_encoder_hidden_states = torch.cat([uncond,cond],dim=0)
        # print(time_aware_encoder_hidden_states.shape)
...
def build_demo(ella_path, sd_path="runwayml/stable-diffusion-v1-5"):
    ...
    ** short prompt also works, but the result is much better after the caption is refined. **
    ...
    got: a mischievous raccoon standing on its hind legs, holding a bright red apple aloft in its furry paws. the apple shines brightly against the backdrop of a dense forest, with leaves rustling in the gentle breeze. a few scattered rocks can be seen on the ground beneath the raccoon's feet, while a gnarled tree trunk stands nearby.
        ...
        with gr.Row():
            with gr.Column():
                gr.Markdown("### ORIGINAL Stable Diffusion Model")
